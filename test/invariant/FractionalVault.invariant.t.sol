...
contract FractionalVaultInvariantTest is Test {
    ...
    function invariant_shareTokenSupplyMatchesTotalShares() public {
        FractionalVault.Vault memory v = vault.getVault(0);
    ...
    function invariant_buyoutPriceDistribution() public {
        ...
        if (v.state == FractionalVault.VaultState.Bought) {
            ...
            uint256 claimedShares = vault.claimedShares(0);
    ...
    function test_stateTransition_ActiveToBought() public {
        ...
        vm.prank(buyer);
        vault.buyout{value: RESERVE_PRICE}(0);
    ...
    function test_claimableAmountCalculation() public {
        ...
        vm.deal(buyer, 20 ether);
        vm.prank(buyer);
        vault.buyout{value: 15 ether}(0);
        
        // Check claimable amounts
        uint256 curatorClaimable = vault.getClaimableAmount(0, curator);
        uint256 sh1Claimable = vault.getClaimableAmount(0, shareholder1);
        uint256 sh2Claimable = vault.getClaimableAmount(0, shareholder2);
