...
contract LazyMinting is EIP712, Ownable, ReentrancyGuard, Pausable {
    ...
    function redeem(
        ...
        if (!authorizedContracts[voucher.nftContract]) revert UnauthorizedContract();
        if (usedNonces[voucher.creator][voucher.nonce]) revert NonceAlreadyUsed();
        ...
        ERC721NFTInitializable nft = ERC721NFTInitializable(voucher.nftContract);
        nft.mint(msg.sender, voucher.uri);
        
        _distributePayment(voucher);
        
        if (msg.value > voucher.price) {
            (bool success, ) = payable(msg.sender).call{value: msg.value - voucher.price}("");
            if (!success) revert TransferFailed();
        }
        
        emit VoucherRedeemed(msg.sender, voucher.creator, voucher.nftContract, voucher.tokenId, voucher.price);
    ...
    function _distributePayment(NFTVoucher calldata voucher) internal {
        ...
        if (platformAmount > 0) {
            (bool feeSuccess, ) = payable(feeRecipient).call{value: platformAmount}("");
            if (!feeSuccess) revert TransferFailed();
        }
        
        if (royaltyAmount > 0) {
            (bool royaltySuccess, ) = payable(voucher.creator).call{value: royaltyAmount}("");
            if (!royaltySuccess) revert TransferFailed();
        }
        
        (bool creatorSuccess, ) = payable(voucher.creator).call{value: creatorAmount}("");
